...
const Settings = () => {
    ...
    const handleUpdatePhone = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Log to Transaction History (Staff Notification)
            await supabase.from('transactions').insert({
                member_id: profile.id,
                type: 'INFO', // New type for logs
                amount: 0,
                description: `会員による電話番号変更: ${profile.phone || '未登録'} → ${phone}`,
                is_cancelled: false,
                served_by: 'SYSTEM'
            });
    ...
    const handleUpdateEmail = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Log to Transaction History (Staff Notification)
            await supabase.from('transactions').insert({
                member_id: profile.id,
                type: 'INFO',
                amount: 0,
                description: `会員によるメールアドレス変更申請: ${profile.email} → ${newEmail}`,
                is_cancelled: false,
                served_by: 'SYSTEM'
            });
