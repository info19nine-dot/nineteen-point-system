...
import { Scan, ChevronRight, Search, QrCode, X, Check, History, PenTool, Plus, Settings as SettingsIcon, ChevronLeft, CheckCircle2, ShieldCheck, AlertCircle, HelpCircle } from 'lucide-react';
    ...
    return (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center gap-2 mb-4">
                     ...
                     <button 
                        ...
                        className="flex items-center gap-1 text-xs font-bold text-teal-600 opacity-60 hover:opacity-100 transition-opacity ml-auto"
                     >
                         <span>すべて見る</span>
                             ...
                             if (isSystem) {
                                 ...
                                 rightContent = (
                                     <>
                                         ...
                                         <div className="text-[10px] text-gray-400 truncate max-w-[120px]">{detail}</div>
                                         <div className="text-[10px] text-gray-400">{displayDate}</div>
                                     </>
                                 );
                             } else {
                                ...
                                // Remove "コース" if present to save space
                                courseName = courseName.replace('コース', '');

                                 rightContent = (
                                     <>
                                         <div className={`font-bold text-sm ${tx.is_cancelled ? 'text-gray-400 line-through' : (tx.type === 'EARN' ? ((tx.profiles as any).rank === 'special' ? 'text-yellow-600 drop-shadow-sm' : 'text-teal-600') : 'text-slate-700')}`}>
                                             {tx.type === 'EARN' ? '+' : '-'}{tx.amount.toLocaleString()} pt
                                         </div>
                                         {/* Compact display: Course Name + Served By */}
                                         {(courseName || tx.served_by) && (
                                            <div className="text-[10px] text-gray-500 font-medium truncate max-w-[120px]">
                                                {courseName} {tx.served_by}
                                            </div>
                                         )}
                                         {tx.is_cancelled && <div className="text-[10px] text-red-500 font-bold leading-tight">取消済</div>}
                                         <div className="text-[10px] text-gray-400">{displayDate}</div>
                                     </>
                                 );
                             }
        ...
        {showHistory && (
            <div className="fixed inset-0 z-50 bg-white animate-in slide-in-from-right duration-300 flex flex-col">
                <div className="p-4 flex items-center gap-4 border-b border-gray-100 bg-white sticky top-0 z-10 safe-area-top">
                    <button 
                    ...
                    <h2 className="font-bold text-lg text-gray-800">取引履歴一覧</h2>
                </div>
                
                <div className="flex-1 overflow-y-auto p-4 pb-24 bg-gray-50">
                    <div className="space-y-3">
                        {recentTransactions.map((tx) => {
                             if (!tx.profiles) return null;
                             const displayDate = new Date(tx.created_at).toLocaleString();
                             const isSystem = tx.amount === 0 && tx.description?.includes('【システム】');
                             ...
                             if (isSystem) {
                                 ...
                                 rightContent = (
                                     <>
                                         <div className="font-bold text-sm text-slate-500">{parts[0]}</div>
                                         <div className="text-[10px] text-gray-400 truncate max-w-[120px]">{parts[1] || ''}</div>
                                         <div className="text-[10px] text-gray-400">{displayDate}</div>
                                     </>
                                 );
                             } else {
                                    let courseName = (tx.description || '')
                                        .replace('【利用】', '')
                                        .replace(/\(元: \d+pt\)/, '')
                                        .replace(/【修正:.*?】/, '') 
                                        .replace(/【調整:.*?】/, '') 
                                    ...
                                    if (tx.description?.includes('Course QR:')) {
                                        courseName = tx.description.replace('Course QR:', '').trim();
                                    }
                                    if (courseName === 'Point usage via QR scan' || courseName === 'QRコード利用') {
                                        courseName = 'ポイント利用';
                                    }
                                    // Remove "コース" if present
                                    courseName = courseName.replace('コース', '');

                                    const isCorrection = tx.description?.includes('【修正') || tx.description?.includes('【調整');
                                    const amountColor = isCorrection 
                                        ? 'text-orange-600 font-bold' 
                                        : (tx.is_cancelled ? 'text-gray-400 line-through' : (tx.type === 'EARN' ? ((tx.profiles as any).rank === 'special' ? 'text-yellow-600 drop-shadow-sm' : 'text-teal-600') : 'text-slate-700'));

                                    rightContent = (
                                        <>
                                            <div className={`font-bold text-sm ${amountColor}`}>
                                                {tx.type === 'EARN' ? '+' : '-'}{tx.amount.toLocaleString()} pt
                                                {isCorrection && <span className="ml-1 text-[10px] bg-orange-100 text-orange-600 px-1 rounded border border-orange-200 align-middle">修正</span>}
                                            </div>
                                            {/* Compact display */}
                                            {(courseName || tx.served_by) && (
                                                <div className="text-[10px] text-gray-500 truncate max-w-[120px]">
                                                    {courseName} {tx.served_by && <span className="text-slate-500">({tx.served_by})</span>}
                                                </div>
                                            )}
                                            {tx.is_cancelled && <div className="text-[10px] text-red-500 font-bold leading-tight">取消済</div>}
                                            <div className="text-[10px] text-gray-400">{displayDate}</div>
                             ...
                             return (
                                 <div 
                                    key={tx.id} 
                                    onClick={() => navigate(`/admin/members/${(tx as any).member_id}`)}
                                    className={`bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center active:scale-95 transition-transform ${tx.is_cancelled ? 'opacity-60' : ''}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold relative ${tx.is_cancelled ? 'bg-gray-200 text-gray-400' : ((tx.profiles as any).rank === 'special' ? 'bg-gradient-to-br from-yellow-300 to-yellow-600 text-white shadow-sm ring-2 ring-yellow-100' : 'bg-slate-100 text-slate-600')}`}>
                                            {tx.profiles.name ? tx.profiles.name[0] : '?'}
                    ...
                    <div className="mt-8 text-center pb-8">
                        <button 
                            ...
                            disabled={isLoading}
                            className="px-8 py-3 bg-white border border-gray-200 rounded-full text-sm font-bold text-slate-600 shadow-sm active:scale-95 transition-all hover:bg-gray-50 disabled:opacity-50"
                        >
