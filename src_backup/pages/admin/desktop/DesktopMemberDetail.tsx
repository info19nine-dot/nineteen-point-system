...
import { ArrowLeft, Loader2, FileText, CheckCircle2, AlertTriangle, Ban, Home } from 'lucide-react';
...
const DesktopMemberDetail = () => {
    ...
    // Pagination State (Fixed limit for now as load more is manual in mobile but distinct here)
    const [txLimit] = useState(20);
    // const [totalTxCount, setTotalTxCount] = useState(0); // Unused for now
    ...
    const fetchMemberData = async () => {
        if (!id) return;
        // Only show loading if initial load or switching members
        if (!member || member.id !== id) setIsLoading(true);
        ...
        // Count unused for now
        // else {
        //      setTotalTxCount(transactions?.count || 0);
        // }
    ...
    const logSystemTransaction = async (title: string, detail: string) => {
        ...
        if (transactions) {
            setMember(prev => prev ? { ...prev, transactions } : null);
        }
    };
    ...
    const executeDemoteMember = async () => {
         ...
         const { error } = await supabase.from('profiles')
            .update({ rank: 'regular', membership_expiry: null, membership_status: 'none' })
            .eq('id', id);
         ...
         if (error) {
             ...
             // Log History
             logSystemTransaction('ランク変更', '特別会員 → 通常会員');
         }
    };
    ...
    if (isLoading) return <div className="p-20 text-center text-slate-400 font-mono animate-pulse">Running Member Check...</div>;
                ...
                <div className="col-span-6 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-full overflow-hidden">
                    <div className="p-6 border-b border-gray-100 flex justify-between items-center flex-shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-xs text-gray-400">全 {member.transactions?.length || 0} 件</span>
                        </div>
                    </div>
                                        ...
                                        return (
                                            <tr key={tx.id} className="hover:bg-slate-50 transition-colors group">
                                                ...
                                                <td className="px-6 py-4 text-center whitespace-nowrap">
                                                    <span className="font-mono text-slate-600 font-bold text-xs opacity-70">
                ...
                <div className="col-span-3 flex flex-col h-full overflow-hidden">
                    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        <h3 className="font-bold text-gray-600 mb-2 flex items-center gap-2 flex-shrink-0">
                         ...
                         <textarea 
                             className="w-full flex-grow p-3 bg-yellow-50/50 rounded-lg border border-yellow-200 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300 resize-none"
                                    ...
                                    <button 
                                        onClick={() => {
                                            if (activeModal.onConfirm) {
                                              ...
                                              // Close? usually logic handles it, but let's assume if onConfirm handles, it might update modal.
                                              // If we close here, we might hide result modal immediately?
                                              // executeAdjustment calls setActiveModal again.
                                              // So we can assume it will overwrite activeModal.
                                            } else {
                                                if (activeModal.type === 'recalc_confirm') executeRecalculatePoints();
                                                // if (activeModal.type === 'delete_confirm') executeSoftDelete(); // Removed
                                        ...
                                        className={`flex-1 py-3 text-white font-bold rounded-xl shadow-lg transition-colors ${
                                            activeModal.type === 'delete_confirm' || activeModal.type === 'suspend_confirm' || activeModal.type === 'cancel_confirm' || activeModal.type === 'confirm' ? 'bg-orange-600 hover:bg-orange-700' : 'bg-slate-800 hover:bg-slate-700'
                                        }`}
                                    >
                                        実行する
                                    </button>
