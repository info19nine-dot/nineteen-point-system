...
import { ChevronLeft, Save, Ban, History, RefreshCw, AlertTriangle, CheckCircle2, AlertCircle, Star, StarOff, User } from 'lucide-react';
...
const SwipeableTransactionItem = ({ tx, isSpecial, onCancel }: { tx: Transaction, isSpecial: boolean, onCancel: (tx: Transaction) => void }) => {
    ...
    const canCancel = !tx.is_cancelled && tx.type !== 'INFO' && !isSystem;
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        if (!isDragging.current || !canCancel) return;
        const diff = e.touches[0].clientX - startX.current;
        // Limit swipe: cannot go right (positive), max left -100
        const newOffset = Math.max(Math.min(diff + currentOffset.current, 0), -100);
        setOffset(newOffset);
    };
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        if (!isDragging.current || !canCancel) return;
        if (e.buttons !== 1) { // If mouse button is released outside
             isDragging.current = false;
             snapToOffset();
             return;
        }
    ...
    const transformStyle = { transform: `translateX(${offset}px)`, transition: isDragging.current ? 'none' : 'transform 0.2s ease-out' };

    return (
        <div className="relative w-full overflow-hidden rounded-lg border border-gray-100 mb-3 bg-red-500 shadow-sm cursor-grab active:cursor-grabbing">
             {/* Background Cancel Button */}
             <div className="absolute top-0 right-0 w-20 h-full flex items-center justify-center text-white font-bold z-0" onClick={() => onCancel(tx)}>
             ...
             {/* Foreground Content */}
             <div 
                className="relative z-10 bg-white w-full p-4 flex justify-between items-start"
                ...
                <div className="flex flex-col gap-1.5 max-w-[60%]">
                     ...
                     <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                            ...
                            : tx.is_cancelled ? 'bg-gray-100 text-gray-500' :
                            tx.type === 'EARN' 
                                ? (isSpecial ? 'bg-yellow-100 text-yellow-700' : 'bg-teal-100 text-teal-700') 
                        ...
                        <span className="text-xs text-slate-400 font-mono">
                            {new Date(tx.created_at).toLocaleString('ja-JP', {year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit'})}
                        </span>
                     </div>

                     {/* Row 2: Title (Course Name) - Large & Normal for EARN/USE, Normal for INFO */}
                     <div className={`leading-tight mt-0.5 ${
                         tx.type === 'INFO' 
                            ? 'text-sm font-normal text-gray-600' 
                            : `font-normal text-base ${tx.is_cancelled ? 'text-gray-400 line-through' : 'text-slate-800'}`
                     }`}>
                        {(() => {
                            // Extract simple title for INFO
                            if (tx.type === 'INFO') {
                                return (tx.description || '').replace('【システム】', '').replace('会員による', '').split(':')[0].trim() || '通知';
                            }
                            ...
                            const desc = (tx.description || '')
                                ...
                                .replace(/\(元: \d+pt\)/, '')
                                .replace('【利用】', '')
                                .replace('Point usage via QR scan', 'ポイント利用')
                                .replace('【システム】', '')
                                .trim();
                     ...
                     {tx.served_by && tx.served_by !== 'SYSTEM' && !tx.served_by.includes('システム') && (
                        <div className="flex items-center gap-1 text-sm text-slate-500 mt-0.5">
                            ...
                            <span>{tx.served_by}</span>
                        </div>
                     )}
                </div>

                {/* RIGHT SIDE */}
                <div className="text-right flex flex-col items-end min-w-[35%]">
                    {/* Amount */}
                    <div className={`font-black text-xl ${
                        tx.is_cancelled 
                            ? 'text-gray-300 line-through' 
                            : (tx.type === 'INFO' ? 'text-gray-400' : (tx.type === 'USE' ? 'text-red-500' : (isSpecial ? 'text-yellow-600' : 'text-teal-600'))) 
                    }`}>
                        {tx.type === 'INFO' ? '' : (tx.type === 'USE' ? '-' : '+') + tx.amount.toLocaleString()} 
                        <span className="text-xs font-normal ml-0.5 opacity-50">pt</span>
                    </div>
                    
                    {/* Balance */}
                    <div className="font-bold text-base text-slate-600 mt-1">
                       {tx.type !== 'INFO' && (tx as any).balance_snapshot !== null && (tx as any).balance_snapshot !== undefined ? (
                            <>
                                <span className="text-xs text-slate-400 font-normal mr-1.5 opacity-80">残高:</span>
                                {(tx as any).balance_snapshot.toLocaleString()}
                            </>
                       ) : (
...
const MemberDetail = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`member_detail_${id}_${Date.now()}`)
            .on(
                'postgres_changes',
                {
                    event: 'INSERT', // Specifically listen for new history
                    schema: 'public',
                    table: 'transactions',
                    filter: `member_id=eq.${id}`
                },
                (_payload: any) => {
                    // console.log('Realtime: New transaction detected!', payload);
                    fetchTransactions(false); // Reload top of list
            ...
            .on(
                'postgres_changes',
                {
                    event: 'UPDATE', // Listen for balance/rank changes
                    schema: 'public',
                    table: 'profiles',
                    filter: `id=eq.${id}`
                },
                (_payload: any) => {
                    ...
                    fetchProfile();
                    fetchTransactions(false); // Also refresh history as memo updates often trigger history
                }
            )
            .subscribe((_status: string) => {
                // console.log(`Realtime Subscription Status (${id}):`, status);
            });
    ...
    const handleSaveMemo = async () => {
        ...
        if (error) {
            ...
            setIsDirty(false);
            // Immediately refresh history (Manual fallback in case Realtime is slow)
    ...
    const executeBlacklistAction = async () => {
        ...
        if (error) {
            setActiveModal({
             ...
             // Log History
             // Log History is now handled by DB Trigger (Memo) only
            ...
            <main className="max-w-md mx-auto px-6 -mt-10 relative z-10 space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-100">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        {/* Header Controls Removed (Visible Count) */}
                    </div>
                        ...
                        {hasMore && (
                            <div className="pt-4 text-center">
                                <button 
                                    ...
                                    className="w-full py-3 bg-slate-50 text-slate-500 font-bold rounded-xl hover:bg-slate-100 flex items-center justify-center gap-2 transition-colors text-sm"
                                >
                                    {isLoadingMore ? (
